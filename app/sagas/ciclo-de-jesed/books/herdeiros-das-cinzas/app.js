(() => {
  ...
  if (isMobileDevice) storage.set("di-cinzas-customized", "0");
  ...
  function emptyPanel(title, text, actionRoute, actionLabel) {
    return `<div class="dark-panel section-card hc-empty-panel"><div><h2>${escapeHtml(title)}</h2><p class="empty-inline">${escapeHtml(text)}</p>${actionRoute ? `<button class="secondary-button" data-route="${escapeHtml(actionRoute)}">${escapeHtml(actionLabel || "Continuar")}</button>` : ""}</div></div>`;
  }
  ...
  function detectPreset() {
    if (storage.get("di-cinzas-customized") === "1") return "custom";
  ...
  function hcAshSvg() {
    return `<svg class="hc-ash-motif" viewBox="0 0 600 400" preserveAspectRatio="xMidYMid slice" aria-hidden="true"><g fill="var(--di-accent)"><path d="M262 175L262 176L260 176L260 175L260 174L261 174L263 173z" opacity=".65"/><path d="M289 160L286 162L284 159L284 157L287 157z" opacity=".5"/><path d="M310 65L306 69L303 67L303 64L305 62L309 61z" opacity=".35"/><path d="M72 258L67 260L64 258L62 253L66 252L71 252z" opacity=".65"/><path d="M224 186L223 187L221 186L222 184L224 184z" opacity=".5"/><path d="M356 29L354 32L352 30L351 29L354 27L356 27z" opacity=".35"/><path d="M360 313L359 315L358 314L358 312L359 312L360 312z" opacity=".65"/><path d="M174 330L173 331L172 332L171 330L172 329L173 329L174 329z" opacity=".5"/><path d="M242 275L240 278L238 276L238 273L241 273z" opacity=".65"/><path d="M437 356L434 357L428 359L428 354L428 349L433 348L437 351z" opacity=".65"/><path d="M299 193L294 197L292 197L289 193L294 191L296 190z" opacity=".65"/><path d="M505 227L504 230L500 227L502 223L504 224z" opacity=".35"/><path d="M189 365L185 368L183 365L182 362L184 360L187 361z" opacity=".35"/><path d="M557 26L555 27L553 26L552 25L555 23L557 23z" opacity=".5"/><path d="M546 243L544 245L542 246L540 243L540 240L544 239L546 240z" opacity=".65"/><path d="M26 106L25 108L23 107L22 107L22 105L25 104L26 105z" opacity=".5"/><path d="M581 261L578 263L575 262L572 257L576 255L581 256z" opacity=".65"/><path d="M203 167L201 168L198 167L199 164L202 164z" opacity=".35"/><path d="M42 289L41 290L39 289L38 287L40 287L42 287z" opacity=".65"/><path d="M252 339L247 340L244 339L241 335L246 331L251 332z" opacity=".65"/><path d="M432 180L431 182L428 182L427 179L430 178L432 177z" opacity=".65"/><path d="M377 330L375 332L373 329L374 327L376 327z" opacity=".5"/><path d="M582 296L580 297L578 296L577 293L582 292z" opacity=".65"/><path d="M201 99L196 101L191 99L191 94L193 90L199 94z" opacity=".35"/><path d="M190 152L188 154L184 156L182 150L185 146L189 148z" opacity=".65"/><path d="M38 138L36 141L34 141L32 138L34 136L35 135L37 136z" opacity=".5"/><path d="M336 353L334 354L330 354L329 351L330 348L332 348L334 349z" opacity=".65"/><path d="M62 330L59 331L58 332L56 329L56 328L59 326L61 327z" opacity=".65"/><path d="M543 21L542 22L540 21L540 19L542 19z" opacity=".5"/><path d="M421 343L417 348L414 345L412 341L417 339L419 341z" opacity=".5"/><path d="M305 191L303 192L301 191L298 190L300 186L302 186L306 188z" opacity=".65"/><path d="M117 379L114 380L112 380L112 377L113 375L116 375z" opacity=".5"/><path d="M179 61L173 64L170 59L171 53L178 54z" opacity=".5"/><path d="M189 109L186 110L183 109L185 105L187 106z" opacity=".35"/></g></svg>`;
  }

  const COVER_CLEAN = "assets/books/ciclo-de-jesed/herdeiros-das-cinzas/cover-clean.webp";

  function renderInicio() {
    const quick = [
      ["characters", "people", "Personagens", "Vozes que ainda serão registadas."],
      ["relationships", "network", "Relações", "Laços, alianças e rupturas, quando escritos."],
      ["timeline", "timeline", "Linha do Tempo", "Os acontecimentos, em ordem, assim que existirem."],
      ["chapters", "chapter", "Capítulos", "Capítulos ainda por escrever."]
    ];
    const book = getBook(BOOK_ID) || {};
    const focus = D.characters.filter(c => c.featured).slice(0, 5).map(character => ({ character, description: character.focus || character.alias || "" }));
    const selectedPlaces = D.places.filter(p => p.featured).slice(0, 6);
    ...
    refs.main.innerHTML = `<div class="page-enter">
      <section class="hero-map-card home-guide-hero hc-home-hero">
        ...
        <div class="hero-map-content">
          ...
          <div class="hero-actions">
            ...
            <button class="secondary-button contemplate-button" data-contemplative="cinzas">${icon("eye")} Modo contemplativo</button>
          </div>
        </div>
        ${book.cover ? `<figure class="hc-hero-cover"><img src="${escapeHtml(book.cover)}" alt="Capa de ${escapeHtml(book.name)}" loading="eager" data-lightbox-image></figure>` : ""}
      </section>
      ...
      ${focus.length ? `<section class="dark-panel section-card home-focus-section"><div class="section-heading"><div><p class="eyebrow">Vidas centrais</p><h2>Personagens em foco</h2><p>As figuras principais de Herdeiros das Cinzas.</p></div><button class="text-button" data-route="characters">Ver todos</button></div><div class="home-focus-grid">${focus.map(({character, description}) => `<button class="home-focus-card" data-route="character/${character.slug}"><span class="home-focus-avatar ${character.image ? 'has-image' : ''}">${character.image ? `<img src="${escapeHtml(character.image)}" alt="Retrato de ${escapeHtml(character.name)}" loading="lazy">` : icon('person')}</span><span><strong>${escapeHtml(character.name)}</strong><small>${escapeHtml(description)}</small></span></button>`).join("")}</div></section>` : ""}
      ${selectedPlaces.length ? `<section class="home-places-section"><div class="section-heading"><div><p class="eyebrow">Território</p><h2>Lugares</h2><p>Cidades, ruínas e regiões essenciais do livro.</p></div><button class="text-button" data-route="places">Ver todos os lugares</button></div><div class="home-place-grid">${selectedPlaces.map(homePlaceCard).join("")}</div></section>` : ""}
      <section class="hc-home-footer">
        ${hcAshSvg()}
        <div class="hc-home-footer-content">
          <p class="hc-inscription">Herdeiros das Cinzas · Livro IV do Ciclo de Jesed</p>
          <div class="hc-footer-links">
            ...
            <button data-contemplative="cinzas">${icon("eye")} Modo contemplativo</button>
  ...
  const externalBookLinks = { "ruinas-dos-ceus": "ruinas.html#/inicio", "guerras-de-sangue": "guerras.html#/dashboard", "dinastia-polar": "dinastia-polar.html#/inicio" };
  function renderBook(id) {
    ...
    refs.main.innerHTML = `<div class="page-enter">
      ...
      <section class="book-detail-hero"><div class="book-detail-cover">${cover}</div><div class="book-detail-copy"><p class="eyebrow">Estado editorial</p><h2>Em preparação</h2><div class="book-full-synopsis">${synopsisHtml(book.synopsis || book.teaser || book.visual)}</div><div class="tag-row"><span class="tag">${D.chapters.length} capítulos escritos</span><span class="tag">${D.maps?.main ? "Mapa disponível" : "Sem mapa ainda"}</span><span class="tag">Em preparação</span></div></div></section>
    </div>`;
    setBreadcrumbs([{label:"Dimensões Infinitas",route:"portal"},{label:"Ciclo de Jesed",route:"inicio"},{label:"Livros",route:"books"},{label:book.name}]);
  }
  ...
  function renderCharacters() {
    ...
    refs.main.innerHTML = `<div class="page-enter">
      ${pageHeader("Pessoas", "Personagens", "Vozes de Herdeiros das Cinzas. As fichas aparecem à medida que o manuscrito avança.")}
  ...
  function renderCharacter(slug) {
    ...
    refs.main.innerHTML = `<div class="page-enter">
      ${pageHeader("Personagem · Herdeiros das Cinzas", "", "", `<button class="secondary-button" data-route="relationships">${icon("network")} Relações</button>`)}
...
function renderRelationships() {
  ...
  refs.main.innerHTML = `<div class="page-enter">${pageHeader("Mundo", "Dinastias", "As dinastias de Jesed em Herdeiros das Cinzas: governo, território, economia e cultura.")}${D.dynasties.length ? `<section class="clan-overview-grid">${D.dynasties.map(dynasty => {const p=dynasty.profile||{};return `<article class="clan-overview-card" data-route="dynasty/${escapeHtml(dynasty.slug)}" tabindex="0" role="link"><div class="clan-overview-copy"><p class="eyebrow">${escapeHtml(dynasty.essence||"Dinastia")}</p><h2>${escapeHtml(dynasty.name)}</h2><p>${escapeHtml(dynasty.summary||"")}</p><div class="tag-row">${p.capital?`<span class="tag">${escapeHtml(p.capital)}</span>`:""}</div></div></article>`;}).join("")}</section>` : emptyPanel("Ainda sem dinastias registadas", "As dinastias de Jesed aparecerão aqui, uma a uma, à medida que forem escritas.")}</div>`;
...
function renderDynasty(slug) {
  ...
  function renderChapters() {
    ...
    refs.main.innerHTML = `<div class="page-enter">
      ${pageHeader("Herdeiros das Cinzas", "Capítulos", "Os capítulos escritos até agora.")}
  ...
  function renderTimeline() {
    ...
    refs.main.innerHTML = `<div class="page-enter timeline-page">${pageHeader("História", "Linha do Tempo", "Cronologia de Herdeiros das Cinzas — apenas o que já está escrito.")}${items.length ? `<section class="timeline-list">${items.map(entry=>`<article class="timeline-item timeline-entry-card" data-route="timeline/${entry.slug}"><div class="timeline-card-heading"><time>${escapeHtml(entry.dateLabel || "Data aproximada")}</time><span>${escapeHtml(entry.category || "Acontecimento")}</span></div><h3>${escapeHtml(entry.name)}</h3><p>${escapeHtml(entry.summary || "")}</p></article>`).join("")}</section>` : emptyPanel("Ainda sem linha do tempo", "Os acontecimentos de Herdeiros das Cinzas aparecerão aqui em ordem cronológica.")}</div>`;
  ...
  // Ordem dos grupos: a ordem em que as dinastias foram registadas; depois as restantes; "sem dinastia" por último.
  function orderedDynastyKeys(keys) {
    const known = (D.dynasties || []).map(item => (item.shortName || String(item.name || '').replace(/^Dinastia\s+/i, '')));
    const first = known.filter(name => keys.includes(name));
    const rest = keys.filter(key => key !== '—' && !first.includes(key)).sort((a, b) => a.localeCompare(b, 'pt-BR'));
    return [...first, ...rest, ...(keys.includes('—') ? ['—'] : [])];
  }
  ...
  function renderPlaces() {
    ...
    const groups = orderedDynastyKeys(Object.keys(byDynasty))
    ...
    refs.main.innerHTML = `<div class="page-enter">${pageHeader("Mundo", "Lugares", "Cidades, capitais, regiões e pontos estratégicos de Jesed em Herdeiros das Cinzas.")}${places.length ? groups : emptyPanel("Ainda sem lugares registados", "Os territórios, cidades e regiões do livro aparecerão aqui.")}</div>`;
  ...
  function renderPlace(slug) {
    ...
    if (place.map) {
      refs.main.querySelector('[data-focus-place]')?.addEventListener("click", () => {
        routeTo("map");
        setTimeout(() => {
          const inst = document.getElementById("hcInteractiveMap")?._diMap;
  ...
  function mountDPMap() {
    ...
    const root = document.getElementById("hcInteractiveMap"); if (!root) return;
  ...
  function renderMap() {
    ...
    if (!map) {
      refs.main.innerHTML = `<div class="page-enter map-page">${pageHeader("Mapa · Livro 4", "Jesed em Herdeiros das Cinzas", "O mapa de Jesed neste livro ainda está a ser preparado.")}${emptyPanel("Mapa ainda não disponível", "O mapa de Jesed na época de Herdeiros das Cinzas aparecerá aqui com marcadores de cidades, regiões e pontos importantes.")}</div>`;
    ...
    const directoryLinks = majorPlaces.map(p =>
      `<button type="button" data-di-global-map-focus="${escapeHtml(p.id)}" data-di-map-root="hcInteractiveMap">${escapeHtml(p.name)}</button>`
    ).join("");
    refs.main.innerHTML = `<div class="page-enter map-page">
      <div id="hcInteractiveMap" class="di-map-shell hc-interactive-map" data-di-map-fullscreen
           style="--di-map-ratio:${escapeHtml(map.ratio)};--di-map-accent:#56708c">
        ...
        <div class="di-map-viewport" data-di-map-viewport tabindex="0"
             aria-label="Mapa interativo de Jesed na época de Herdeiros das Cinzas. Arraste para deslocar, roda do rato para ampliar.">
  ...
  function loreKindInfo(kind) {
    return {
      fauna: ["Fauna", "Animais e criaturas do livro.", "paw"],
      flora: ["Flora", "Plantas, raízes e fibras do livro.", "leaf"],
      foods: ["Alimentos", "Preparações e alimentos cerimoniais ou cotidianos.", "bowl"],
      concepts: ["Conceitos e Leis", "Símbolos, costumes e princípios do mundo.", "scroll"]
  ...
  function renderSettings() {
    const s = state.settings;
    refs.settingsContent.innerHTML = `<div class="settings-group"><h3>Perfis rápidos</h3><div class="preset-row">${[["full","Completo"],["normal","Equilibrado"],["performance","Desempenho"],["custom","Personalizado"]].map(item => `<button class="preset-button ${s.preset === item[0] ? "active" : ""}" data-preset="${item[0]}">${item[1]}</button>`).join("")}</div></div><div class="settings-group"><h3>Controlo individual</h3>${settingToggle("particles","Cinzas e partículas","Cinzas a cair, fumaça e elementos suspensos.",s.particles)}${settingToggle("transitions","Transições entre páginas","Véu de cinza entre uma página e outra.",s.transitions)}${settingToggle("textures","Texturas e gravações","Grão, névoa e marcas de cinza.",s.textures)}${settingToggle("blur","Desfoque de painéis","Efeito de vidro nas barras e janelas.",s.blur)}${settingToggle("shadows","Sombras profundas","Profundidade dos cartões e painéis.",s.shadows)}${settingToggle("motion","Movimento e animações","Entradas e movimentos suaves.",s.motion)}<div class="setting-row range-row"><span class="setting-copy"><strong>Quantidade de cinzas</strong><small>Ajuste fino do fundo.</small></span><input type="range" min="4" max="50" value="${s.particleAmount}" data-setting-range="particleAmount"></div></div>`;
  }
  ...
  function openSelector(type) {
    ...
    const hrefFor = item => books
      ? (item.id === "ruinas-dos-ceus" ? "ruinas.html#/inicio" : item.id === "guerras-de-sangue" ? "guerras.html#/dashboard" : item.id === "dinastia-polar" ? "dinastia-polar.html#/inicio" : "")
  ...
  document.addEventListener("click", event => {
    ...
    const setting = event.target.closest("[data-setting]"); if (setting) { const key = setting.dataset.setting; state.settings[key] = !state.settings[key]; storage.set("di-cinzas-customized","1"); persistSettings(); }
    const preset = event.target.closest("[data-preset]"); if (preset) {
      state.settings.preset = preset.dataset.preset;
      if (state.settings.preset === "custom") { storage.set("di-cinzas-customized","1"); persistSettings(); }
      else {
        storage.set("di-cinzas-customized","0");
  ...
  refs.settingsContent.addEventListener("input", event => { if (event.target.matches("[data-setting-range]")) { state.settings[event.target.dataset.settingRange] = Number(event.target.value); storage.set("di-cinzas-customized","1"); persistSettings(); } });
